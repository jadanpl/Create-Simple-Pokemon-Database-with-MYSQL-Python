...
def add_pokemon():
  ...
  db_cursor.execute("INSERT INTO dragon_pokemon (Pokemon, FastMove, ChargeMove, DPS, TDO, Total) VALUES (%s, %s, %s,%s, %s, %s)", (Pokemon, FastMove, ChargeMove, DPS, TDO, Total))
  database.commit()
...
def update_pokemon():
  ...
  TDO = input("Enter TDO: ")
  Total = input("Enter Total: ")
  db_cursor.execute("UPDATE dragon_pokemon SET Pokemon = %s, FastMove = %s, ChargeMove = %s DPS = %s, TDO = %s, Total = %s WHERE id = %s", (Pokemon, FastMove, ChargeMove, DPS, TDO, Total, id))
  database.commit()
